;window.CloudflareApps=window.CloudflareApps||{};CloudflareApps.siteId="51cbe9539301cb61132964374707462e";CloudflareApps.installs=CloudflareApps.installs||{};;(function(){'use strict'
...
return false}}());CloudflareApps.installs["qZtzki1ZGyJ3"]={appId:"mt9QUlJFKGSg",scope:{}};;CloudflareApps.installs["qZtzki1ZGyJ3"].options={"account":{"accountId":"PXGKYZv0AUDs","service":"jsguardian"},"free":{"guardInnerHTML":"strict","guardInput":"clean","guardPrompt":"prevent","guardTransport":"https-only","guardWhen":"before","guardXSS":"sanitize"}};;if(CloudflareApps.matchPage(CloudflareApps.installs['qZtzki1ZGyJ3'].URLPatterns)){(function(){"use strict";function cloneObject(obj,done=new Map()){if(done.has(obj)){return done.get(obj);}
...
return"";}
var _addEventListener=window.addEventListener,_setAttribute=Element.prototype.setAttribute,options=cloneObject(typeof(CloudflareApps.installs['qZtzki1ZGyJ3'].options)!=="undefined"?CloudflareApps.installs['qZtzki1ZGyJ3'].options:{"free":{"guardWhen":"before","guardInnerHTML":"clean","guardPrompt":"clean","guardInput":"clean","guardTransport":"https-only","guardXSS":"sanitize"},"plan":"free"});function isExecutable(str){var functionExp=/([\s\(]*.\(\s*.*\s*\)\s*.*\s*=>)|([\s*.*\s*=>\s*.\{)|([\s\(]*.[Ff]unction\s*.*\s*\(\s*.*\s*\)\s*.*\s*\{)/,templateExp=/(\`(\s*.*)\$\{(\s*.*\s*.*)\}(\s*.*)\`)/;return functionExp.test(str)||templateExp.test(str)};function protect(){if(options.guardInput&&options.guardInput!=="off"){(function(){if(_addEventListener){_addEventListener.call(window,"change",function(event){var target=event.currentTarget;if(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement){var oldvalue=target.getAttribute("value"),clean=sanitize(target.value);if(oldvalue===clean){return;}
